...
namespace Rendering
{
   ...
   LineRenderer::LineRenderer(
    ...
    void LineRenderer::Update(Windows::Foundation::Numerics::float3 positionA, Windows::Foundation::Numerics::float3 positionB, const Graphics::StepTimer& /* timer */)
       
    {
		...
		//float Scale_x = abs(vector_line.x);
		//float Scale_y = abs(vector_line.y);
		//float Scale_z = abs(vector_line.z);

        // Position the marker.
       auto modelTranslation =
		...
		float radiansY = static_cast<float>(fmod(_rotationInRadiansY, DirectX::XM_2PI));
		...
		float radiansZ = static_cast<float>(fmod(_rotationInRadiansZ, DirectX::XM_2PI));

		

		dbg::trace(L"TargetPosition");
		dbg::trace(L" %f %f %f ", _positionA.x, _positionA.y, _positionA.z);
		...
		XMStoreFloat4x4(&trans, modelScale);
		...
		XMStoreFloat4x4(&trans, modelTransformNotScaled);

		dbg::trace(L"not scaled");
		dbg::trace(L"%f %f %f %f ", trans.m11, trans.m12, trans.m13, trans.m14);
		dbg::trace(L"%f %f %f %f ", trans.m21, trans.m22, trans.m23, trans.m24);
		dbg::trace(L"%f %f %f %f ", trans.m31, trans.m32, trans.m33, trans.m34);
		dbg::trace(L"%f %f %f %f ", trans.m41, trans.m42, trans.m43, trans.m44);
    ...
    void LineRenderer::CreateDeviceDependentResources()
    {
        ...
        // Once all shaders are loaded, create the mesh.
        {
...

            {
				...
				{ {
				{ { 0.f, 0.f, 0.f},{ 1.0f, 1.0f, 1.0f },{ 1.0f, 1.0f } },
				{ { s, 0.f , 0.f},{ 1.0f, 1.0f, 1.0f },{  1.0f, 1.0f } },
				} };
    ...
    void LineRenderer::ReleaseDeviceDependentResources()
    {
